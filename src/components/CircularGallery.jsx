...
class App {
  ...
  findMediaAtClientX(clientX) {
    if (!this.medias || !this.medias.length || !this.container) return null;
    const rect = this.container.getBoundingClientRect();
    if (!rect.width) return null;
    const normalized = (clientX - rect.left) / rect.width - 0.5;
    const worldX = normalized * this.viewport.width;
    let best = null;
    let bestDistance = Infinity;
    for (const media of this.medias) {
      const distance = Math.abs(media.plane.position.x - worldX);
      const halfWidth = media.plane.scale.x / 2;
      if (distance <= halfWidth + 0.15 && distance < bestDistance) {
        best = media;
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new App(containerRef.current, {
        ...
        scrollSpeed,
        scrollEase,
        onItemClick: (data) => clickRef.current && clickRef.current(data)
      });
    });
